...
import {Login, GetCurrUserInfo} from '../../serviceApi/auth'
  ...
  methods: {
    ...
    async onLogin() {
      ...
      Login(params).then((res =>{
        if (res.code == 1 && res.msgCode!='FAIL'){
          sessionStorage.setItem("token", res.data.token);
          GetCurrUserInfo().then(res =>{
            if (res.code == 1) {
              this.$store.state.user.userInfo = res.data;//全局缓存当前用户信息
              sessionStorage.setItem("userInfo", JSON.stringify(res.data));
              this.$store.commit("user/getPermissionPointMenuInfo_sync");  
